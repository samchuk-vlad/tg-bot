...
import { AccountInfoItem, CrowdloanInfo, RelayChain } from './type';
...
export const getAccountInfo = async (account: string) => {
  const promises = supportedNetworks.map(async network => {
    try {
      const { data: info, status } = await axios.get(getBackendUrl(`${account}/balances/${network}`))
      if (status !== 200) {
...
export const getCrowdloans = async ({ account, relayChain }: GetCrowdloansProps) => {
  try {
    const res = await axios.get(getBackendUrl(`crowdloans/contributions/${relayChain}/${account}`))
...
export const getCrowdloansInfoByRelayChain = async (relayChain: RelayChain): Promise<CrowdloanInfo[]> => {
  try {
    ...
    console.error('Failed to get crowdloans info by relay chain', relayChain, err)
    return []
  }
}
